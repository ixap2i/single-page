...
const createIonRouter = (opts, router) => {
        ...
        if (routeInfo && routeInfo.pushedByRoute) {
            ...
            if (prevInfo) {
                ...
                if (routeInfo.lastPathname === routeInfo.pushedByRoute ||
                    ...
                    router.replace({ path: prevInfo.pathname, query: vueRouter.parseQuery(prevInfo.search) });
                }
            }
            else {
    ...
    const resetTab = (tab, originalHref) => {
        ...
        if (routeInfo) {
            ...
            router.push({ path: newRouteInfo.pathname, query: vueRouter.parseQuery(newRouteInfo.search) });
        }
    };
    const changeTab = (tab, path) => {
        ...
        if (routeInfo) {
            ...
            if (routeInfo.pathname === pathname) {
                router.push({ path: routeInfo.pathname, query: vueRouter.parseQuery(routeInfo.search) });
            }
            else {
                router.push({ path: pathname, query: vueRouter.parseQuery(routeInfo.search) });
            }
        }
        else {
...
const createViewStacks = (router) => {
    ...
    const createViewItem = (outletId, vueComponent, matchedRoute, routeInfo, ionPage) => {
        return {
            ...
            vueComponent,
            vueComponentRef: vue.shallowRef(),
            ionRoute: false,
            mount: false,
...
const createWebHistory = (base) => vueRouter.createWebHistory(base);
const createWebHashHistory = (base) => vueRouter.createWebHashHistory(base);
const createMemoryHistory = (base) => vueRouter.createMemoryHistory(base);
